...
<body>
  <div class="wrap">
    <div class="tabs">
      ...
      <button class="tab" id="tabWork">Working</button>
    </div>
    ...
    <div class="panel" id="panelSetup">
      <div class="bar">
        ...
        <label><input type="checkbox" id="dbgRx"/> Debug RX Logs</label>
    ...
    <div class="panel hidden" id="panelWork">
      <div class="grid2">
        <div>
          ...
          <div class="bar">
            ...
            <input id="manPayload" style="min-width:320px" value="FA FA 00 00" />
...
function rowHtml(p) {
  return `<tr id="r${p.id}">
    ...
    <td id="p${p.id}_1"></td>
    <td id="p${p.id}_2"></td>
    <td><input class="small" id="n${p.id}"/></td>
    <td><button class="write-now-btn" onclick="queueWrite(${p.id})" disabled>Write Now</button></td>
    <td>${p.range}</td>
  </tr>`;
}
...
function showTab(which) {
  ...
  work.classList.toggle('hidden', onSetup);
  tabSetup.classList.toggle('active', onSetup);
  tabWork.classList.toggle('active', !onSetup);
}

document.getElementById('tabSetup').onclick = () => showTab('setup');
document.getElementById('tabWork').onclick = () => showTab('work');

const ws = new WebSocket(`ws://${location.hostname}:81`);
ws.onopen = () => { setStatus('Connected'); addLog('WebSocket connected'); applyWriteButtonsState(); };
ws.onclose = () => { setStatus('Disconnected'); addLog('WebSocket disconnected'); setMode('OUT'); applyWriteButtonsState(); };
...
ws.onmessage = (ev) => {
  ...
  if (m.startsWith('DEBUG:')) {
    dbgRxEl.checked = m.slice(6).trim() === '1';
    return;
  }
...
dbgRxEl.onchange = () => send(`DEBUG_RX:${dbgRxEl.checked ? 1 : 0}`);
...
void wsDebugState() {
  wsServer.broadcastTXT(String("DEBUG:") + (debugCanRxLogs ? "1" : "0"));
}
...
bool shouldLogCanId(uint32_t id) {
  ...
  return id == KEYA_RESPONSE_ID || id == KEYA_CONFIG_ID;
}
...
void handleCommand(const String& text) {
  ...
  if (text.startsWith("DEBUG_RX:")) {
    String v = text.substring(9);
    v.trim();
    debugCanRxLogs = (v == "1" || v.equalsIgnoreCase("true") || v.equalsIgnoreCase("on"));
    wsStatus(String("Debug RX logs ") + (debugCanRxLogs ? "enabled" : "disabled"));
    wsDebugState();
    return;
  }
...
void onWebSocketEvent(uint8_t num, WStype_t type, uint8_t* payload, size_t length) {
  if (type == WStype_CONNECTED) {
    ...
    wsServer.sendTXT(num, String("DEBUG:") + (debugCanRxLogs ? "1" : "0"));
